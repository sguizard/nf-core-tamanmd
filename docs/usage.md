# nf-core/tamanmd: Usage

## :warning: Please read this documentation on the nf-core website: [https://nf-co.re/tamanmd/usage](https://nf-co.re/tamanmd/usage)
...
```bash
nextflow run nf-core/tamanmd --input samplesheet.csv --genome GRCh37 -profile docker
```
...
First, go to the [nf-core/tamanmd releases page](https://github.com/nf-core/tamanmd/releases) and find the latest version number - numeric only (eg. `1.3.1`). Then specify this when running the pipeline with `-r` (one hyphen) - eg. `-r 1.3.1`.
...
* `docker`
  ...
  * Pulls software from Docker Hub: [`nfcore/tamanmd`](https://hub.docker.com/r/nfcore/tamanmd/)
* `singularity`
